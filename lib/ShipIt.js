...
var writeConfig = function(basePath) {
  ...
  exists(configPath, function(is_exists) {
    if(is_exists) {
      console.warn('.shipit.json is already exists');
    } else {
      ...
      fs.writeFile(configPath, bt(JSON.stringify(config)), function(err) {
        ...
        console.info('Success to create configuration file : ' + configPath);
      });
    }
  });
};
...
var State = function(cwd, release, options) {
  ...
  var version, curVersion, changeLogs = [], vc, mainFilePath;
...
var releasePackage = function(cwd, release, options) {
  ...
  function(config, cb) {
    ...
    var superCb = cb;
    steps.push(function(cb) {
      console.info('Success to ship it');
      cb(null);
      return superCb(null);
    });
    steps.push(function(cb) {
    ...
    if(err) {
      ...
      boolPrompt('Rollback?', 'y', function(ans) {
        if(ans) {
          console.info('Rollbacking executed steps...');
...
exports.parseAndRun = function() {
  var cmd = require('commander').version(exports.version).option('-x, --exec', 'Run command actually(default dry-run mode)', false);

  //Init
  cmd.command('init').description('Initialize: write default configuration file to ./.shipit.json').action(function() {
    writeConfig(process.cwd());
  });

  //Release
  cmd.command('release [release-type]').description('Release the package to NPM. release-type is [major|minor|patch|build|custom].').action(function(releaseType) {
    var isExec;
